...
// TEMPORARILY DISABLED FOR CADENCE 1.0 MIGRATION
...
access(all) contract Toucans {
  ...
  // Helper function to build type identifiers for ToucansActions
  ...
  access(all) resource Project: ProjectPublic {
    ...
    access(all) fun finalizeAction(actionUUID: UInt64) {
      ...
      if actionState == ActionState.ACCEPTED {
        ...
        let action = actionWrapper.getAction()
        let actionTypeId = action.getType().identifier
        
        // Use if-else instead of switch for Cadence 1.0 compatibility
        ...
        } else if actionTypeId == Toucans.getActionTypeId("BatchWithdrawToken") {
        ...
        } else if actionTypeId == Toucans.getActionTypeId("WithdrawNFTs") {
        ...
        } else if actionTypeId == Toucans.getActionTypeId("MintTokens") {
        ...
        } else if actionTypeId == Toucans.getActionTypeId("BatchMintTokens") {
        ...
        } else if actionTypeId == Toucans.getActionTypeId("BurnTokens") {
        ...
        } else if actionTypeId == Toucans.getActionTypeId("MintTokensToTreasury") {
        ...
        } else if actionTypeId == Toucans.getActionTypeId("AddOneSigner") {
        ...
        } else if actionTypeId == Toucans.getActionTypeId("RemoveOneSigner") {
        ...
        } else if actionTypeId == Toucans.getActionTypeId("UpdateTreasuryThreshold") {
        ...
        } else if actionTypeId == Toucans.getActionTypeId("LockTokens") {
        ...
        } else if actionTypeId == Toucans.getActionTypeId("StakeFlow") {
            // TEMPORARILY DISABLED FOR CADENCE 1.0 MIGRATION
            panic("StakeFlow action temporarily disabled")
        } else if actionTypeId == Toucans.getActionTypeId("UnstakeFlow") {
            // TEMPORARILY DISABLED FOR CADENCE 1.0 MIGRATION
            panic("UnstakeFlow action temporarily disabled")
        }
      }
    ...
    access(all) fun purchase(paymentTokens: @{FungibleToken.Vault}, projectTokenReceiver: &{FungibleToken.Receiver}, message: String) {
      ...
      let emeraldCityTreasury = getAccount(0x5643fd47a29770e7).capabilities.get<&{FungibleToken.Receiver}>(self.paymentTokenInfo.receiverPath).borrow()
    ...
    // TEMPORARILY DISABLED FOR CADENCE 1.0 MIGRATION
    // TODO: Re-enable when SwapInterfaces is compatible
    // access(account) fun stakeFlow(flowAmount: UFix64, stFlowAmountOutMin: UFix64) {
    //   panic("stakeFlow temporarily disabled for Cadence 1.0 migration")
    // }

    // TEMPORARILY DISABLED FOR CADENCE 1.0 MIGRATION
    // TODO: Re-enable when SwapInterfaces is compatible
    // access(account) fun unstakeFlow(stFlowAmount: UFix64, flowAmountOutMin: UFix64) {
    //   panic("unstakeFlow temporarily disabled for Cadence 1.0 migration")
    // }
  ...
  access(all) resource Collection: CollectionPublic {
    ...
    access(CollectionOwner) fun voteOnProjectAction(projectOwner: Address, projectId: String, actionUUID: UInt64, vote: Bool) {
      let collection: &Collection = getAccount(projectOwner).capabilities.get<&Toucans.Collection>(Toucans.CollectionPublicPath).borrow()
  ...
  access(all) resource MultiSignAction {
      ...
      access(all) view fun getActionState(): ActionState {
        ...
        if self.action.getType().identifier == Toucans.getActionTypeId("AddOneSigner") {
      ...
      init(_threshold: UInt64, _signers: [Address], _action: AnyStruct) {
  ...
  access(all) resource Manager: ManagerPublic {
    ...
    access(account) fun createMultiSign(action: AnyStruct) {
      var threshold: UInt64 = self.threshold
      var signers: [Address] = self.signers
      if action.getType().identifier == Toucans.getActionTypeId("AddOneSigner") {
