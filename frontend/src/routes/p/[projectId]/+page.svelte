<script type="ts">
	...
	import { setDaoContextFromProject, clearDaoContext } from '$lib/features/dao-agent/utils/setDaoContext';
	...
	onMount(() => {
		...
		// Set DAO context for AI agent
		setDaoContextFromProject($daoDataStore, $user.addr);
	});

	onDestroy(() => {
		// Clear DAO context when leaving the page
		clearDaoContext();
	});

	// Update context when user changes or DAO data updates
	$: if ($daoDataStore && $user.addr) {
		setDaoContextFromProject($daoDataStore, $user.addr);
	}
</script>
